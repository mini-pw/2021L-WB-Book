 ...
 * creation of a text-detecting mask using the threshold function from the OpenCV library (detection of the brightest spots in the photo),
 * modifying (thickening) the mask using the dilate function from OpenCV (for greater effectiveness),
 * removing all text from the image using the mask created in the earlier steps and inpaint function from OpenCV.

We have also decided to drop the part of the authors' code where they introduce the division between the right and left side of the image, as they did not use this later in the code and it does not seem necessary.
