---
output: rmarkdawn::github_document
...
W pracy http://export.arxiv.org/pdf/2003.09871 autorzy stosują sieć COVID-Net na tym samym zbiorze danych COVID-X. Porównują swoje wyniki z wynikami, które uzyksali na sieciach VGG-19 oraz ResNet-50. Różni się to od DeepCOVIDExplainer przede wszystkim tym, że w latter autorzy dokonali praktycznie ensemble trzech sieci, zaś w COVID-Net używają tylko tytułowej.

https://arxiv.org/pdf/2004.04582.pdf?fbclid=IwAR0zSSiGZCFk2JSRK97pL59mfbYvyMMkSJJudB4QIEjY96upknm8fEw2lio 

https://github.com/rezacsedu/DeepCOVIDExplainer <- podlinkowane w artykule repo
...
```{r VGG19, out.width="600", fig.align="center", echo=FALSE, fig.cap='VGG19 architecture'}
knitr::include_graphics('images/3-3-vgg19.png')
```
...
```{r ResNet18, out.width="600", fig.align="center", echo=FALSE, fig.cap='ResNet18 architecture'}
...
```{r DenseNet161, out.width="600", fig.align="center", echo=FALSE, fig.cap='DenseNet161 architecture'}
...
```{r BeforeAfter, out.width="600", fig.align="center", echo=FALSE, fig.cap='Before and after preprocessing'}
...
```{r ConfMatrix, out.width="600", fig.align="center", echo=FALSE, fig.cap='Confusion matrix for base model '}
...
```{r Smallchanges, out.width="600", fig.align="center", echo=FALSE, fig.cap='Confusion matrix for model with small changes'}
knitr::include_graphics('images/3-3-small_changes.png')
```

As the images we use to train the network must be in RGB format, we also trained each channel of the already loaded images separately using the Fast Fourier Transform. The results obtained were not an improvement over baseline, but they remain in the 70-80% accuracy range and thus are not a significant degradation of the model.
...
```{r L1L2, out.width="600", fig.align="center", echo=FALSE, fig.cap='Confusion matrix for model with L1, model with L2 and model with both'}
...
```{r WihoutDropout, out.width="600", fig.align="center", echo=FALSE, fig.cap='Confusion matrix for model without dropout'}
...
```{r CM1, out.width="600", fig.align="center", echo=FALSE, fig.cap='Confusion matrix for model with branching in dense layers'}
...
```{r CM2, out.width="600", fig.align="center", echo=FALSE, fig.cap='Confusion matrix for model with class weights mechanism'}
...
```{r CM3, out.width="600", fig.align="center", echo=FALSE, fig.cap='Confusion matrix for model with separated first convolutional block'}
...
```{r Ensemble, out.width="600", fig.align="center", echo=FALSE, fig.cap='Ensemble of ResNet and VGG'}
...
```{r Ensemble2, out.width="600", fig.align="center", echo=FALSE, fig.cap='Ensemble of multiple VGG'}
...
```{r GAN, out.width="600", fig.align="center", echo=FALSE, fig.cap='GAN generator results'}
...
```{r Transfer, out.width="600", fig.align="center", echo=FALSE, fig.cap='Transfer learning results'}
...
```{r Aux, out.width="600", fig.align="center", echo=FALSE, fig.cap='Auxilary task results'}
...
```{r Auth, out.width="600", fig.align="center", echo=FALSE, fig.cap='Original project authors' results'}
