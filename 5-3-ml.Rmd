...
```{r fig6, out.width="500", fig.align="center", echo=FALSE, fig.cap='Principal Component Analysis'}
...
```{r fig7, out.width="500", fig.align="center", echo=FALSE, fig.cap='Principal Component Analysis with line separator'}
...
```{r fig8, out.width="750", fig.align="center", echo=FALSE, fig.cap='Original Data distribution'}
knitr::include_graphics('images/5-3-hist1.png')
```

```{r fig9, out.width="750", fig.align="center", echo=FALSE, fig.cap='New Data distribution'}
...
```{r fig10, out.width="500", fig.align="center", echo=FALSE, fig.cap='Skewed data results'}
knitr::include_graphics('images/5-3-skeweddataresults_excel.png')
```

From figure \@ref(fig:fig10) we can see that the model generally improved (the accuracy is higher) and it perform slightly better.
...
In order to achieve our goal we create an easy application, in which one can choose his or hers chronic diseases, sex, age and it calculates the COVID-19 mortality risk for particular infected person. Additionally, in the bookmarks there are presented plots about the model. The first one \@ref(fig:fig12) is a Break Down plot which shows how the contributions attributed to individual explanatory variables change the mean models prediction. Despite printing out the risk it also enables its user an easy option to understand the outcome. Another one \@ref(fig:fig13) is Ceteris Paribus profile which examines the influence of an explanatory variable (which is `age` in this case) by assuming that the values of all other variables do not change. This visualization is very useful for doctors to properly distinguish a higher risk groups. The last one \@ref(fig:fig14) is a SHAP plot, it calculates the importance of a feature by comparing what a model predicts with and without the feature.

Our application is avaiable here: https://hubertr21.shinyapps.io/Explainable_COVID19_Mortality_Predictor/

```{r fig12, out.width="750", fig.align="center", echo=FALSE, fig.cap='Mortality Breakdown profile'}
knitr::include_graphics('images/5-3-mortalBreakdown.png')
```

```{r fig13, out.width="750", fig.align="center", echo=FALSE, fig.cap='Mortality Ceteris Paribus profile'}
knitr::include_graphics('images/5-3-mortalParibus.png')
```

```{r fig14, out.width="750", fig.align="center", echo=FALSE, fig.cap='Mortality SHAP profile'}
knitr::include_graphics('images/5-3-mortalSHAP.png')
```

### Conclusions

```{r fig15, out.width="750", fig.align="center", echo=FALSE, fig.cap='XAI stakeholders from [@5-3-XAI_et_al]'}
knitr::include_graphics('images/5-3-XAI_stakeholders.jpg')
```

Thanks to the developed application, various XAI stakeholders, as presented in [@5-3-XAI_et_al] - \@ref(fig:fig15), may have easier access to transparent and explainable model that estimates mortality risk in case of being infected by COVID-19. Our application allows people to see which diseases are contributing the most to the outcome, without the need of doctor's examination, such as any blood properties and other information, that can only be gathered by a specialist are not necessary. Only by selecting diseases that one suffers from, reliable prediction can be obtained quickly and without leaving home.
